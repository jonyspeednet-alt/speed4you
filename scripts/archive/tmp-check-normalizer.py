...
r = subprocess.run(['psql', '-U', 'postgres', '-d', 'isp_portal', '-c', "SELECT value FROM app_state WHERE key='media_normalizer_state'"], capture_output=True, text=True, env={'PGPASSWORD': 'postgres'})
