# Helper script to generate GitHub ***REMOVED***s commands
...
foreach ($varName in $optionalVars.Keys) {
    $value = [Environment]::GetEnvironmentVariable($varName)
    if ([string]::IsNullOrWhiteSpace($value)) {
        $***REMOVED***s[$varName] = $optionalVars[$varName]
    } else {
        $***REMOVED***s[$varName] = $value
    }
}
...
$commandParts = @()
foreach ($key in $***REMOVED***s.Keys) {
    $commandParts += "gh ***REMOVED*** set $($key) --body `"$($***REMOVED***s[$key])`""
}

if ($envFilePath) {
    $commandParts += "gh ***REMOVED*** set DEPLOY_ENV_FILE_CONTENT --body `"$([System.IO.File]::ReadAllText($envFilePath))`""
} else {
