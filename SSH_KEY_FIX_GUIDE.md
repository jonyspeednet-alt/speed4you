...
ssh-keygen -p -f deploy_key -N "" -P "***REMOVED***"
```
...
- `-P "***REMOVED***"`: Old passphrase

### Step 2: Verify Key Works

```bash
# Test SSH connection (should NOT ask for ***REMOVED***)
...
1. Go to: https://github.com/jonyspeednet-alt/speed4you-portal/settings/***REMOVED***s/actions
2. Click on `DEPLOY_SSH_KEY`
3. Copy content of `deploy_key` file (entire private key)
4. Paste it into the ***REMOVED***
5. Click "Update ***REMOVED***"
...
- [ ] SSH connection works without ***REMOVED***:
  ```bash
  ssh -i deploy_key -p 2973 speed4you@***REMOVED*** "echo OK"
  ```
- [ ] GitHub ***REMOVED*** `DEPLOY_SSH_KEY` updated with new private key
...
ssh -i deploy_key -p 2973 speed4you@***REMOVED*** "cat ~/.ssh/authorized_keys"

# Check key permissions on server
ssh -i deploy_key -p 2973 speed4you@***REMOVED*** "ls -la ~/.ssh/"
```
...
2. **Keep deploy_key secure** - it's like a ***REMOVED***
