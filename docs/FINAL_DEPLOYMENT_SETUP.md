...
This will automatically set all ***REMOVED***s except `DEPLOY_ENV_FILE_CONTENT`.
...
Click "New repository ***REMOVED***" and add these 8 ***REMOVED***s:
...
```bash
ssh -i deploy_key -p 2973 speed4you@***REMOVED*** "echo 'SSH Connection Successful!'"
```
...
# Check if all ***REMOVED***s are properly set
```

### Test 2: Manual SSH Test
```bash
ssh -i deploy_key -p 2973 speed4you@***REMOVED*** "ls -la /home/speed4you/"
```
...
3. Test connection: ssh -i deploy_key -p 2973 speed4you@***REMOVED***
```
...
- `setup-github-***REMOVED***s.ps1` - Automated ***REMOVED***s setup script
...
2. **Setup Server** (Run commands on ***REMOVED***)
