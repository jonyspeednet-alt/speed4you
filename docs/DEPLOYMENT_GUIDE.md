...
Recommended configuration for newer Nginx versions on your production server `***REMOVED***`:
...
After reloading Nginx, check if HTTP/2 is active on server `***REMOVED***` using curl:
```bash
