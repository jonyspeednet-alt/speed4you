...
- **GitHub repository** admin or maintainer access to configure repository secrets and webhooks.
...
GitHub Secrets store sensitive deployment credentials that the GitHub Actions workflow references at runtime. Secrets are never exposed in logs and are only available to workflows running on the repository. All secrets must be configured before the first deployment attempt; missing any required secret will cause the workflow to fail.
...
4. Enter the secret name exactly as listed below (case-sensitive).
5. Paste the secret value and click **Add secret**.

### Adding Secrets via GitHub CLI

If you have the [GitHub CLI (`gh`)](https://cli.github.com/) installed and authenticated, you can add secrets from the terminal:

```bash
gh secret set DEPLOY_HOST -b "<YOUR_SERVER_IP>"
gh secret set DEPLOY_PORT -b "<YOUR_SSH_PORT>"
gh secret set DEPLOY_USER -b "<YOUR_SSH_USERNAME>"
gh secret set DEPLOY_SSH_KEY < <PATH_TO_YOUR_PRIVATE_KEY>
gh secret set JWT_SECRET -b "<YOUR_JWT_SECRET>"
gh secret set TMDB_API_KEY -b "<YOUR_TMDB_API_KEY>"
gh secret set DEPLOY_SUDO_PASSWORD -b "<YOUR_SUDO_PASSWORD>"
gh secret set DEPLOY_REMOTE_CORS_ALLOWED_ORIGINS -b "<YOUR_PRODUCTION_CORS_ORIGIN>"
gh secret set DEPLOY_REMOTE_PLAYER_CACHE_ROOT -b "<YOUR_PLAYER_CACHE_PATH>"
gh secret set DEPLOY_ENV_FILE_CONTENT -b "<YOUR_ENV_FILE_CONTENT>"
```
...
| `DEPLOY_SUDO_PASSWORD` | Sudo password for the SSH user (required for service restarts) | `<YOUR_SUDO_PASSWORD>` |
...
- **Never commit secrets** to the repository, even in `.env.example` files. Use placeholder values in example files.
- **Rotate secrets periodically**, especially after any team member with access leaves the project.
- **Use a strong JWT secret** — the application enforces a minimum of 32 characters in production and rejects known weak values such as `changeme` or `secret`.
- **Limit repository access** — only grant write access to trusted collaborators who need to trigger deployments.
- **Audit secret usage** — periodically review which secrets are defined and remove any that are no longer needed.

---

## 4. SSH Key Setup

SSH key-based authentication is the backbone of the automated deployment pipeline. The GitHub Actions workflow and the local webhook listener both rely on SSH keys to connect to the server without interactive password prompts. This section covers generating a new key pair, installing the public key on the server, adding the private key to GitHub Secrets, and verifying the connection end-to-end.

### Step 1: Generate an Ed25519 Key Pair

Ed25519 is the recommended key type for modern SSH deployments. It provides stronger security with shorter keys compared to RSA and is supported by all current OpenSSH versions. **Generate the key without a passphrase**, because automated CI/CD processes cannot provide interactive password input.
...
Copy the entire contents of the `deploy_key` file (including the `-----BEGIN OPENSSH PRIVATE KEY-----` and `-----END OPENSSH PRIVATE KEY-----` lines) and add it as the `DEPLOY_SSH_KEY` secret in your GitHub repository settings. See [Section 3](#3-github-secrets-setup) for detailed instructions on adding secrets.

### Step 4: Test the SSH Connection

From your local machine, verify that the key works without prompting for a password:
...
9. The `.env` file is written from the `DEPLOY_ENV_FILE_CONTENT` secret.
...
This is the content you should set as the `DEPLOY_ENV_FILE_CONTENT` GitHub secret. The deployment workflow writes this verbatim to the server's `.env` file:
...
- **Weak JWT secrets** (less than 32 characters or matching known default values like `changeme`) will be rejected.
...
5. If using `DEPLOY_ENV_FILE_CONTENT` in GitHub Secrets, update the secret to include the correct `PORT` value so future deployments do not overwrite it with the wrong port.
...
6. **Check the `DEPLOY_SSH_KEY` GitHub secret** — make sure it includes the entire private key file content, including the BEGIN and END lines, with no extra whitespace or line breaks added accidentally.
...
| **Rotate JWT secret** | Quarterly | Update `JWT_SECRET` in `.env` and `DEPLOY_ENV_FILE_CONTENT` secret, then restart |
