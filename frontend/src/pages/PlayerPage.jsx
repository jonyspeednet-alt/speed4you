...
function PlayerPage() {
  ...
  const hasEpisodeSelection =
    searchParams.has("season") || searchParams.has("episode");

  const inferredDuration =
    Number(content?.durationSeconds || 0) ||
    normalizeDuration(
      content?.duration || content?.runtimeMinutes || content?.runtime || 0,
    );
  const trustedMediaDuration = isPlausibleMediaDuration(
    mediaDuration,
    inferredDuration,
  )
    ? mediaDuration
  ...
  const progressPercent = activeDuration
    ? clamp((activeTime / activeDuration) * 100, 0, 100)
    : 0;
  ...
  const upgradeToOptimizedStream = useCallback(
    (baseUrl) => {
      ...
      const resumeFrom = Number.isFinite(currentVideo.currentTime)
        ? currentVideo.currentTime
        : 0;
      const optimizedUrl = withPlayerApiBase(
        `${baseUrl}${baseUrl.includes("?") ? "&" : "?"}requireOptimized=1&_ts=${Date.now()}`,
      );
  ...
  const scheduleInvisibleVideoCheck = useCallback(() => {
    ...
    invisibleVideoCheckRef.current = window.setTimeout(() => {
      ...
      const playbackTime = Number.isFinite(video.currentTime)
        ? video.currentTime
        : 0;

      if (
        playbackTime >= 1.5 &&
        (!hasKnownDimensions || renderedFrames === 0)
      ) {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      switch (e.key.toLowerCase()) {
        ...
        case "arrowleft":
          e.preventDefault();
          preSeekTimeRef.current = video.currentTime;
      ...
      try {
        ...
        if (!cancelled) {
          setContent({
            ...
            durationSeconds:
              stream.episode?.durationSeconds ||
              stream.durationSeconds ||
              item.durationSeconds ||
              0,
            runtimeMinutes:
              stream.episode?.runtimeMinutes ||
              stream.runtimeMinutes ||
              item.runtimeMinutes ||
              item.runtime ||
              null,
            duration:
              stream.episode?.duration || item.duration || item.runtime || 0,
            runtime:
              stream.episode?.duration || item.runtime || item.duration || 0,
          ...
          setQualityLabel(
            String(
              primarySource.qualityLabel ||
                primarySource.quality ||
                primarySource.resolution ||
                "Auto",
            ),
          );
          ...
          if (
            ...
            (async () => {
              ...
              for (let attempt = 0; attempt < 40 && !cancelled; attempt += 1) {
                const prepare = await playerService.prepareStream(
                  item.type,
                  contentId,
                  {
                    season: seasonNumber,
  ...
  useEffect(() => {
    async function loadResume() {
      try {
        ...
        const saved = await progressService.getFor(
          content.type,
          contentId || 1,
        );
  ...
  useEffect(() => {
    ...
    async function setupAudioBoost() {
      try {
        ...
        gainNodeRef.current.gain.value = isAudioBoostEnabled
          ? audioBoostLevel
          : 1;
  ...
  const playNextEpisode = useCallback(() => {
    ...
    if (content?.type === "series") {
      navigate(
        `/play/${contentId}?season=${content.season}&episode=${content.episode + 1}`,
      );
    }
  ...
  const persistProgress = useCallback(
    async (position, duration) => {
      ...
      if (
        Math.abs(position - lastSavedPositionRef.current) < RESUME_SAVE_INTERVAL
      ) {
        return;
      }
      ...
      try {
        await progressService.update(
          ...
          contentId,
          Math.floor(position),
          Math.floor(duration || activeDuration || position),
        );
      } catch {
  ...
  useEffect(() => {
    ...
    function onKeyDown(event) {
      ...
      if (event.key === " " || event.key === "Enter") {
        ...
        preSeekTimeRef.current = videoRef.current.currentTime;
  ...
  const handleSurfaceTouchMove = (event) => {
    ...
    if (Math.abs(dx) > 30 && Math.abs(dx) > Math.abs(dy)) {
      ...
      if (skipSeconds !== 0) {
        ...
        setGestureFeedback(
          skipSeconds > 0 ? `+${skipSeconds}s` : `${skipSeconds}s`,
        );
        if (swipeTimeoutRef.current) clearTimeout(swipeTimeoutRef.current);
      }
      ...
      if (isRightSide) {
        ...
        const newBrightness = Math.max(
          0.2,
          Math.min(1, brightness + change * 2),
        );
    ...
    return (
      <div style={styles.state}>
        <div style={styles.stateCard}>
          ...
          <Link to="/" style={styles.backToPortal}>
            Back to portal
          </Link>
        </div>
      </div>
  ...
  return (
    <div
      style={{ ...styles.page, cursor: showControls ? "default" : "none" }}
    ...
    >
      <video
        ...
        onTimeUpdate={(event) => {
          ...
          const nextDuration = Number.isFinite(event.target.duration)
        ...
        onDurationChange={(event) => {
          if (
            isPlausibleMediaDuration(event.target.duration, inferredDuration)
          ) {
        ...
        onSeeked={(event) => {
          ...
          if (event.target.currentTime < 0.5 && pendingSeekTargetRef.current > 5) {
            event.target.currentTime = preSeekTimeRef.current;
          }
          pendingSeekTargetRef.current = null;
        }}
        onEnded={() => {
        ...
        <div style={styles.autoPlayOverlay}>
          <div style={styles.autoPlayBox}>
            ...
            <div style={styles.autoPlayButtons}>
              <button
                style={styles.autoPlayBtnCancel}
                onClick={() => setAutoPlayCountdown(null)}
              >
                Cancel
              </button>
              <button style={styles.autoPlayBtnNext} onClick={playNextEpisode}>
                Play Now
              </button>
            </div>
          </div>
        ...
        >
          <div style={styles.rippleCircle}>
            <svg
              ...
              aria-hidden="true"
              style={
                rippleData.side === "left" ? { transform: "scaleX(-1)" } : {}
              }
            >
      ...
      {toastData && (
        <div style={styles.toastContainer}>
          ...
          <div style={styles.toastTrack}>
            <div
              style={{ ...styles.toastFill, width: `${toastData.value}%` }}
            />
          </div>
        </div>
      )}
      ...
      {gestureFeedback && (
        <div style={styles.gestureFeedback}>{gestureFeedback}</div>
      )}

      <div
        style={{ ...styles.chrome, ...(isMobile ? styles.chromeMobile : {}) }}
      />
      <div
        style={{
          ...styles.vignette,
          ...(isMobile ? styles.vignetteMobile : {}),
        }}
      />
                      ...
                      <div className="yt-settings-list">
                        ...
                        {streamMode === "optimized" && (
                          <div className="yt-settings-item disabled">
                            ...
                            <span
                              className="yt-settings-item-value"
          ...
          >
            ...
            <div style={styles.mobileScrubRow}>
              <div style={styles.mobileScrubberWrap}>
                <div className="yt-mobile-scrubber-track">
                  <div
                    className="yt-mobile-buffered-track"
                    style={{ width: `${bufferedPercent}%` }}
                  />
                  <div
                    className="yt-mobile-progress-track"
              ...
              <div style={styles.mobileTimeRow}>
                <span style={styles.mobileTimeText}>
            ...
            <div style={styles.mobileActionRow}>
              ...
              >
                ...
                <span style={styles.mobileActionLabel}>
                  {isMuted ? "Unmute" : "Mute"}
                </span>
              </button>
              ...
              <label style={styles.mobileActionBtn} aria-label="Playback speed">
                ...
                <select
                  ...
                  style={styles.mobileHiddenSelect}
              ...
              <button
                style={{
                  ...
                  ...(isAudioBoostEnabled ? styles.mobileActionBtnActive : {}),
                }}
...
const styles = {
  ...
  chrome: {
    ...
    zIndex: 5,
    background:
      "linear-gradient(180deg, rgba(0,0,0,0.72) 0%, transparent 22%, transparent 60%, rgba(0,0,0,0.88) 100%)",
    pointerEvents: "none",
  },
  chromeMobile: {
    background:
      "linear-gradient(180deg, rgba(0,0,0,0.65) 0%, transparent 18%, transparent 55%, rgba(0,0,0,0.92) 100%)",
  },
  vignette: {
    position: "absolute",
    top: 0,
    ...
    zIndex: 5,
    background:
      "radial-gradient(circle at center, transparent 48%, rgba(0,0,0,0.46) 100%)",
    pointerEvents: "none",
  },
  ...
  autoPlayCounter: {
    ...
    lineHeight: 1,
    textShadow: "0 4px 16px rgba(255, 200, 87, 0.4)",
  },
  autoPlayButtons: {
    display: "flex",
    gap: "16px",
  ...
  mobileTopBar: {
    ...
    gap: "8px",
    padding:
      "env(safe-area-inset-top, 16px) max(16px, env(safe-area-inset-right)) 16px max(16px, env(safe-area-inset-left))",
    background:
