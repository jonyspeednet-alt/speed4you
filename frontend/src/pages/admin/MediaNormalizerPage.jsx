        ...
        <div style={s.leftCol}>
          ...
          <div style={s.card}>
            <div style={s.cardHeader}>
              ...
              {currentProgress && (
                <span style={s.eta}>ETA: {eta || 'calculating...'}</span>
              )}
            </div>
            ...
            {currentProgress ? (
              <div style={s.progressSection}>
                ...
                <div style={s.progressTrack}>
                  <div style={{ ...s.progressFill, width: `${pct}%`, background: pct > 90
                ...
                {currentProgress.fps && (
                  <div style={s.progressFps}>{currentProgress.fps} fps</div>
                )}
              </div>
            ) : (
              <div style={s.emptyState}>
                ...
                <span>No active conversion. Press <strong>Start</strong> to begin processing media files.</span>
              </div>
            )}
          </div>
            ...
            <div style={s.card}>
              ...
              <div style={s.workersGrid}>
                <div style={currentProgress ? s.workerCard : s.workerCardIdle}>
                  ...
                  {currentProgress && (
                    <div style={s.workerPhase}>{currentProgress.phase}</div>
                  )}
                </div>
          ...
          <div style={s.card}>
            <div style={s.cardHeader}>
              ...
              <span style={s.historyCount}>Last 5</span>
            </div>
            <div style={s.historyList}>
              {(normalizer.recentLogLines || []).filter(l => l.includes('done') || l.includes('failed') || l.includes('skip-ok')).slice(-5).length > 0
                ? (normalizer.recentLogLines || []).filter(l => l.includes('done') || l.includes('failed') || l.includes('skip-ok')).slice(-5).map((line, i) => {
                  const type = logType(line);
                  return (
                    <div key={i} style={s.historyItem}>
                      <span style={{ ...s.historyDot, background: type === 'error' ? '#f87171' : type === 'success' ? '#4ade80' : '#94a3b8' }} />
                      <span style={s.historyText}>{line.split(']').pop()?.trim() || line}</span>
                    </div>
                  );
                })
                : <div style={s.emptyState}>
                    <span style={{ fontSize: '0.85rem' }}>No conversions yet.</span>
                  </div>
              }
            </div>
          </div>
...
const s = {
  ...
  cardTitle: {
    ...
    fontSize: '0.95rem', fontWeight: '700', color: TEXT, margin: 0,
  },
  ...
  progressGlow: {
    height: '100%', borderRadius: '999px', position: 'absolute', top: 0, left: 0,
  ...
  emptyState: {
    ...
    padding: '32px', color: TEXT3, fontSize: '0.88rem', textAlign: 'center',
  },
  ...
  // History
  historyList: { display: 'flex', flexDirection: 'column', gap: '6px', maxHeight: '240px', overflowY: 'auto' },
  historyCount: { fontSize: '0.72rem', color: TEXT3, fontWeight: '600' },
  historyItem: { display: 'flex', alignItems: 'flex-start', gap: '10px', padding: '10px 12px', borderRadius: '8px', background: SURFACE2, fontSize: '0.8rem' },
  historyDot: { width: '8px', height: '8px', borderRadius: '50%', marginTop: '5px', flexShrink: 0 },
  historyText: { color: TEXT2, lineHeight: 1.4, wordBreak: 'break-all' },

  // Log
  logFilters: { display: 'flex', gap: '4px', flexWrap: 'wrap' },
  logFilterBtn: {
    padding: '4px 10px', borderRadius: '6px', fontSize: '0.72rem', fontWeight: '700',
    cursor: 'pointer', border: 'none', background: 'transparent', color: TEXT3,
    transition: 'all 150ms ease',
  },
  logFilterActive: { background: SURFACE2, color: TEXT2, border: `1px solid ${BORDER}` },

  logBox: {
    padding: '12px', minHeight: '300px', maxHeight: '480px', overflowY: 'auto',
    background: '#0d1117', borderBottomLeftRadius: '16px', borderBottomRightRadius: '16px',
    fontFamily: 'ui-monospace, SFMono-Regular, Menlo, monospace', fontSize: '0.75rem', lineHeight: 1.8,
  },
