...
const SECRET = process.env.JWT_SECRET || 'isp-***REMOVED***-key-change-in-production';

router.post('/login', async (req, res) => {
  const { username, ***REMOVED*** } = req.body;
  
  if (!username || !***REMOVED***) {
    return res.status(400).json({ error: 'Username and ***REMOVED*** required' });
  }
