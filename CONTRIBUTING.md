...
3. **Configure environment variables:**
   - Create a `.env` file in the `backend/` directory based on the required environment variables for database connection, JWT secret, and other configuration options. Refer to the backend configuration module at `backend/src/config/` for the complete list of expected variables.
...
- Include browser console errors for frontend issues and server logs for backend issues. Redact any sensitive information such as passwords or tokens before sharing.
